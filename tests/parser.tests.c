...
int should_fail_to_parse_integer_atom_that_is_too_large(void)
{
    ...
    parser_t parser;
    program_t program = {0};
    const char *input = "1234567890123456789012345678901234567890";
    int err = parser_init(&parser, input, strlen(input));
    if (err) return 1;
    err = parser_parse(&parser, &program);
...
int should_fail_to_parse_float_atom_that_is_too_large(void)
{
    ...
    parser_t parser;
    program_t program = {0};
    const char *input = "1234567890123456789012345678901234567890.12345678901234567890";
    int err = parser_init(&parser, input, strlen(input));
    if (err) return 1;
    err = parser_parse(&parser, &program);
    if (err != PARSER_ERR_NUMBER_TOO_LARGE) {
...
int should_parse_list_of_atoms(void)
{
    ...
    parser_t parser;
    program_t program = {0};
    const char *input = "(1 2.5 \"hello\" my_symbol)";
    int err = parser_init(&parser, input, strlen(input));
    if (err) return 1;
    err = parser_parse(&parser, &program);
    if (err) {
...
int should_parse_a_simple_lisp_program(void)
{
    ...
    parser_t parser;
    program_t program = {0};
    const char *input = "(define (factorial n) (if (= n 0) 1 (* n (factorial (- n 1)))))";
    int err = parser_init(&parser, input, strlen(input));
    if (err) return 1;
    err = parser_parse(&parser, &program);
    if (err) {
...
int should_parse_multiple_top_level_forms(void)
{
    ...
    parser_t parser;
    program_t program = {0};
    const char *input = "(define x 1) (define y 2)";
    int err = parser_init(&parser, input, strlen(input));
    if (err) return 1;
    err = parser_parse(&parser, &program);
    if (err) {
